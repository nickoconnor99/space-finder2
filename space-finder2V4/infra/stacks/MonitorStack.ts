...
export class MonitorStack extends Stack {
  constructor(scope: Construct, id: string, props?: StackProps) {
    ...
    const alarmTopic = new Topic(this, 'AlarmTopic', {  //SNS Topic
      displayName: 'AlarmTopic',
      topicName: 'AlarmTopic'
    })
    
    //everytime topic receives and event it will trigger lambda
    ...
    const topicAction = new SnsAction(alarmTopic)  //SNS action

    //use action to trigger our alarm
    spacesApi4xxAlarm.addAlarmAction(topicAction)

    //action for when enteres ok state
